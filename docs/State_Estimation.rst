...
The final discretized matricies were implemented as numpy arrays as follows:

  .. math::

    A_d = \begin{bmatrix}
        0.499445 & 0.499445 & 0.001942 & 0.002727 \\
        0.499445 & 0.499445 & 0.001942 & -0.002727 \\
        0.285397 & 0.285397 & 0.001110 & -0.000000 \\
...
.. math::
    ...
    B_d = \begin{bmatrix}
        0.143168 & 0.140021 & 0.000545 & 0.000765 & -0.142699 & -0.142699 \\
        0.140021 & 0.143168 & 0.000545 & -0.000765 & -0.142699 & -0.142699 \\
        -0.142699 & -0.142699 & 0.499445 & 0.000000 & -0.000000 & 0.000000 \\
        -2.012050 & 2.012050 & 0.000000 & 0.022074 & -0.000000 & 0.000000
    \end{bmatrix}

.. math::

    C = \begin{bmatrix}
        0.000000 & 0.000000 & 0.000000 & -0.248227 \\
        0.000000 & 0.000000 & 0.000000 & 0.248227 \\
        1.000000 & 1.000000 & 0.000000 & 0.000000 \\
        -70.500000 & 70.500000 & 1.000000 & 0.000000
    \end{bmatrix}

Each time the state estimator task runs, it updates all input values with measured values from the IMU and Encoders; these are used to update an augmented u matrix - containing all values of y as well as the left and right motor voltages. The core of the state estimator is a discrete-time linear observer that updates the state estimate according to

.. math::

    \hat{x}_{k+1} = A_d \, \hat{x}_{k} + B_d \, u_k^\ast

where:

- :math:`\hat{x}_k` is the current estimated state vector,
- :math:`A_d` is the discrete-time state transition matrix,
- :math:`B_d` is the discrete-time input matrix, and
- :math:`u_k^\ast` is an augmented input vector containing motor voltages and
  measured outputs (wheel displacements, yaw angle, and yaw rate).

This update equation predicts the next state :math:`\hat{x}_{k+1}` by combining
the dynamics encoded in :math:`A_d` with the current sensor and actuator inputs
through :math:`B_d`. The observer runs every cycle of the state estimator task,
continually refining the robot’s internal estimate of wheel velocities, traveled
distance, and heading.

Implementation in the Multitasking Structure
-----------------------------------------------
